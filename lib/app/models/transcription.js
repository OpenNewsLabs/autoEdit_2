...
module.exports = Backbone.Model.extend({
  ...
  returnPlainTextTimecoded: function(attr, frequency = 1, condense = false) {
    var result;
    ...
    function createPlainTextTimecoded(srtJsonContent, frequency = 1, condense = false, cb) {
      var lines = '';
      ...
      // var newLineText = condense ? '' : '\n'

      //
      for (var i = 0; i < srtJsonContent.length; i++) {

        var srtLineO  = srtJsonContent[i];
        var startTime = condense ? srtLineO.startTime.slice(0, -4) : srtLineO.startTime
        var endTime   = condense ? '' : '\t' + srtLineO.endTime
        var speaker   = '\t' + srtLineO.speaker

        if(condense && speaker == '\t' + "Unnamed Speaker") speaker = ''
    ...
    loopThroughStuff(attr.text, function(res) {
      result = createPlainTextTimecoded(res, frequency, condense);
    });
