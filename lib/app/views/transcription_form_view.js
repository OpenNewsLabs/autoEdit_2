...
module.exports = Backbone.View.extend({
  ...
  events: {
    ...
    // for browser only 
    'change #inputMediaFile': 'electronGetFilePath'
  },
  ...
  // TODO if used by both browser, electron and CEP rename as something more sensible
  electronGetFilePath: function(e) {
    ...
    if(window.ENV_BROSWER){
      ...
      // TODO: Need to trigger a oper file dialogue programmatically. 
      // check out client side template input code hidden there
      // altho I think it just uses another html5 browser input and not the electron one in browser.
      
      self.newFilePath = e.currentTarget.files[0]
    }
  ...
  save: function(e) {
    ...
    // if(window.ENV_CEP || window.ENV_ELECTRON){
      ...
      } else if (sttEngine === 'assemblyai') {
        newLanguage = document.querySelector('#languageModelAssemblyai').value;
      ...
      if (newFilePath == '') {
        ...
        if (sttEngine === 'ibm' || sttEngine === 'speechmatics' || sttEngine === 'BBC' || sttEngine ==='assemblyai') {
          if (navigator.onLine) {
