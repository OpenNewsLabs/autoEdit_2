...
module.exports = Backbone.View.extend({
  ...
  events:{
    "click button.deletePendingBtn": "deletePending"
  },
  render: function() {
    // if there are  transcriptions it shows
    this.$el.empty();
    this.$el.append("<ol class='breadcrumb'><li class='active'>Transcriptions</li>");

    // add remove processing transcripts button
    this.$el.append(`
        <button type="button" class="btn btn-danger btn-xs deletePendingBtn">
          Delete Pending Transcripts <span class="glyphicon glyphicon-trash" aria-hidden="true"></span>
        </button>
      </ol>`);
  ...
  deletePending: function() {
    for (var i = 0; i <  appTranscription.transcriptionsList.models.length; i++) {
