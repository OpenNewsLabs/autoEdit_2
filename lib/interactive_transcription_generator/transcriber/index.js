...
var parseGoogle                 = require('./google_stt/parse.js');
var SendToGoogle                = require('./google_stt/send_to_google.js');
var convertGoogleToJson         = require('./google_stt/convert_to_json.js');
var writeOutGoogle              = require('./google_stt/write_out.js');
/**
 * @function transcribe
...
var ibmMaxLengthSlit = 60*5; //seconds 
var googleMaxLengthSplit = 50; //seconds
...
//TODO: refactor this with promisses?
 ...
 var transcribe = function(config) {
  ...
  var audioFileForIBM  = audioFile.split(".ogg")[0]+".tmp.flac";

  if(config.sttEngine == 'google'){
    audioFile = audioFileForIBM;
  }
  ...
  convertToWav(file, audioFile, ffmpegPath, function(newFile) {
    ...
    var ibmSttExtension = 'ogg';

    split(newFile,ibmMaxLengthSlit, tmpFolder, ffmpegPath, ffprobePath, ibmSttExtension, function(files) {
      ...
      total = files.length; 
        ...
        files.forEach(function(f) {
          ...
          sendToWatsonUtil.send(f.name, config.keys, config.languageModel, function(error, data) {
      ...
      //if chose to use gentle offline for speech to text
       ...
       //TODO: else if config.sttEngine == "srt"
      }else if(config.sttEngine == "google"){
         console.info("YOU HAVE CHOSEN Google");

         console.info(JSON.stringify(config,null,2));

        var SendToGoogleUtil = new SendToGoogle();
        var googleSttExtension = 'flac';
        // var out               = [];
        // var total             = 0;
        var googleKeys = {apiKey: config.keys.apiKey};//{projectId: config.keys.project_id, keyFilename: config.keys.path, credentials: config.keys };
        //TODO: move max length for split somewhere that is not a magic number
        split(newFile,googleMaxLengthSplit, tmpFolder, ffmpegPath, ffprobePath, googleSttExtension, function(files) {
            ...
            files.forEach(function(f) {
              ...
              SendToGoogleUtil.send(f.name, googleKeys, config.languageModel, function(err, data){
                ...
                if(data!= null){
                  ...
                  //parse
                  var parsed = parseGoogle(data, f.offset);
                  console.log("parsed", JSON.stringify(parsed, null, 2));

                  out.push({ f: f, data: parsed });
                  total--;
                  if (total === 0) {
                      // callback(null, convertToJson( writeOut( out )));
                      callback(null, convertGoogleToJson( writeOutGoogle( out )) );
                  }
      ...
      }else if(config.sttEngine == "microsoft"){
         console.info("YOU HAVE CHOSEN Microsoft ");
        console.info(JSON.stringify(config,null,2));
