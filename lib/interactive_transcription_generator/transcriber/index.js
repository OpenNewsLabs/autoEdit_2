...
var makeBBCKaldiTranscription   = require('./bbc/send_to_bbc.js');
var getRevTranscriptJson      = require('./rev/index.js');
var convertRevTranscription   = require('./rev/convert-json.js');
...
const convertAssemblyaiJsonToTranscripJson = require('./assemblyai/convert_to_json.js')
...
//  }else{
    ...
    convertToWav(file, audioFile, ffmpegPath, function(newFile) {
      ...
      } else if (config.sttEngine =="assemblyai"){
        console.log("assemblyai config.sttEngine: ",JSON.stringify(newFile,null,2));
        assemblyai.setAPIKey(config.keys.assemblyai.password);

        async function upload (fileToUpload) {
          try {
            const instance = new assemblyai.Upload(fileToUpload)
            const response = await instance.create()
            const data = response.get()
            // do something with the data
            console.log(data)
            callback(null, convertAssemblyaiJsonToTranscripJson(data));
          } catch (e) {
      ...
      } else if (config.sttEngine =="BBC"){
        console.log("BBC config.sttEngine: ",JSON.stringify(newFile,null,2));
        // TODO: replace userEmail
        console.log('config.keys.bbc.email', config.keys.bbc.email);
        makeBBCKaldiTranscription(newFile, config.keys.bbc.email).then((transcription)=>{
          callback(null, transcription);
        }).catch((error)=>{
          callback(error, null);
          console.error('inside send_to_bbc error: ',error);
      })
