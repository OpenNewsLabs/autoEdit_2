...
//TODO: refactor this with promisses?
 ...
 var transcribe = function(config) {
  ...
  convertToWav(file, audioFile, ffmpegPath, function(newFile) {
    ...
    split(newFile, tmpFolder, ffmpegPath, ffprobePath, ibmSttExtension, function(files) {
      ...
      //if chose to use gentle offline for speech to text
       ...
       //TODO: else if config.sttEngine == "srt"
        ...
        var out               = [];
        var total             = 0;
        var googleKeys = {projectId: config.keys.project_id, keyFilename: config.keys.path };
        split(newFile, tmpFolder, ffmpegPath, ffprobePath, googleSttExtension, function(files) {
            //iterate [optional]
            // reading number of audio files 
            total = files.length; 
            //3. iterate and send to STT
