        ...
        if (!error && response.statusCode == 200) {
            ...
            r.on('response',  function (res) {               
                if (response.statusCode === 200) {
                    ...
                    var stream = res.pipe(mediaWriteStream);
                    // https://stackoverflow.com/questions/11447872/callback-to-handle-completion-of-pipe
                    stream.on('finish', function () { 
                        console.log('Finished STREAM downloading file, call callback here ', mediaFileDestination);
                        if(cb){
                            cb(mediaFileDestination)
                        }
                    ...
                    //     console.log('Finished downloading file, call callback here ', mediaFileDestination);
                    //     if(cb){
                    //         cb(mediaFileDestination)
                    //     }
                    // });
